...
import fr.xebia.mow.model.controler.impl.AvoidNonEmpty;
import fr.xebia.mow.model.controler.impl.MoveCommand;
import fr.xebia.mow.model.impl.APositionnedObject;

/**
 * @author Pierre GAUTIER <pierre.pg.gautier@gmail.com>
 *
 */
public class TestMowControlerMoveNonempty extends ATestMowControler {

    @Override
    public void customizeBefore() {
        this.map.add(new APositionnedObject(Point.create(5, 4)) {
        });
        this.map.add(new APositionnedObject(Point.create(4, 5)) {
        });
        this.controler.addRule(new AvoidNonEmpty());
    }
